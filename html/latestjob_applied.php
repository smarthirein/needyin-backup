...
if(isset($_POST['apply']))
{
   ...
   if($sc!=0)
   {
			 ...
			 if($count ==0)
			{
					...
					if($irr_relav['emp_id']){
		             $insert_jexp ="INSERT INTO tbl_applied SET JUser_Id='".$juserid."',emp_id='".$empid."',JobId='".$jobid."',status='yes',relavent='yes',user_status='".$ss_data['JuserStatus']."'";
					}
					else{
						$insert_jexp ="INSERT INTO tbl_applied SET JUser_Id='".$juserid."',emp_id='".$empid."',JobId='".$jobid."',status='yes',relavent='no',user_status='".$ss_data['JuserStatus']."'";
					}
		   	
			         $insert_jexp1 ="INSERT INTO tbl_recent_views SET userid='".$juserid."',Action='Applied',Activity='Applied Job',Reference='".$jobid."',empid='".$_POST['empid']."',Ipaddress='".$_SERVER['REMOTE_ADDR']."',user_status='".$ss_data['JuserStatus']."'";
					 $jexp1= mysqli_query($con,$insert_jexp1);
					 $jexp= mysqli_query($con,$insert_jexp);
			   $desi_name="select Desig_Id,Desig_Name from tbl_desigination where Desig_Id='".$rr_data['Job_Name']."'";
				$desi= mysqli_query($con,$desi_name); 
				$desin=mysqli_fetch_array($desi);
				// echo $desin['Desig_Name']; exit;
              ...
              $insert_query = "INSERT into tbl_notifications SET job_id='".$jobid."',description='".$description."',job_owner_id='".$empid."',profile_id='".$juserid."',notification_to='".$empid."',notification_from='".$juserid."',mode='jobseeker'"; 
					...
					$subject="Job Application";
	                 ...
	                 $message .= "Thanks,<br>Team Needyin<br>";
if($ss_data['JuserStatus']=='A'){
					 ...
					 $message2 .= "Thanks for applying new job on <a href='".$siteurl."'>needyin.com </a>!<br /><br />";
	                 $message2 .= "Job Details : <br />";
	                 $message2 .= "Job Title : ".$row2s['Desig_Name']."<br />";
	                 $message2 .= "Company Name : ".$result_cj5['Comp_Name']."<br /><br />We've shared the information with the concerned recruiter. In case your profile suits to the requirement, recruiters will be in touch with you directly or you will know the feedback/status of your application soon.<br><br>";
	                 $message2 .= "Just click following link !<br /><br />";
	                 $message2 .= "<a href=".$siteurl.">NeedyIn </a><br /><br />";
	                 $message2 .= "Thanks,<br>Team Needyin<br>";       
				...
				else 
				{
					...
					window.location.href = "job-search-results-postlogin.php";</script>
					<?php  
				}
		}
		else
