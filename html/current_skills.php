...
$skills=$_POST['skills'];
$skill_query="SELECT Sec_Skills from tbl_jobseeker where JUser_Id='".$_SESSION['userSession']."' ";
$skill_res=mysqli_query($con,$skill_query);
$skill_row=mysqli_fetch_array($skill_res);
$secondary_skills=explode(",",$skill_row['Sec_Skills']); 
if(array_intersect($skills,$secondary_skills)){?>
...
{?>
<script language="javascript">alert("Please Select  Skill to update");
window.location.href="jobseeker-profile.php"; </script>
...
$sec_skills=implode(",",$secondary_skills); 
if($sec_skills == '')
{ 
	$skill_ids=$askills;
}
else
$skill_ids=$askills.",".$sec_skills;
}
else
{
...
if($skill_ids!=$pre_skills)
{
		...
		$jobs=array_unique($job_ids);
			...
			$user_update_query = "UPDATE tbl_jobseeker SET pri_skills='".$askills."',Job_Skills='".$skill_ids."' where JUser_Id='".$_POST['user_id']."' "; 
